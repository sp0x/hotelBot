...
model_directory = "./model/default/model_20190508-130514"
...
class DialogFlow:
    ...
    def next(self):
        ...
        if len(remaining_boxes) == 0:
            if self.has_finished_all_suggestions():
                ...
                next_random_suggestions = self.add_suggestions(True, exclusions, next_random_query, custom_radius=max_radius, cnt=3)
    ...
    def process_message(self, message):
        ...
        intent, ents = parse_intent(message)
        lower = message.lower()
        if (("here" in lower) and "already" in lower) or \
                ("there" in lower and "now" in lower):
            intent = "visit"
            ents = {'DATE': ['Today']}
        elif intent in place_intents:  # Handle interests without entities
            if len(ents) == 0:
                ents['interest'] = [intent]
